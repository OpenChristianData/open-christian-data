...
7. **American English in all project output.** Project uses American English.
...
| `contributors` | array | No | Array of `{name, role, note?}` objects. Roles: `author`, `translator`, `editor`, `series_editor`, `reviser`, `transcriber`, `compiler`, `contributor`, `narrator`, `annotator`, `digitizer`. Use `note` for clarifying text (e.g. `"General Editor, 1908-1914 edition"`). Prefer over a bare `author` field when attribution is multi-party or role-specific. |
| `edition` | string | No | Edition or version identifier (e.g. `"2nd ed. 1559"`, `"Second Series"`, `"1662"`). |
...
Book identity (`book`, `book_osis`, `book_number`) lives at the file level since files are book-scoped. Envelope `meta.scope.book_osis` carries this. `book_number` uses 1-66 for the standard Protestant order and 67+ for apocrypha/deuterocanon books when a source canon includes them.
...
| `work_kind` | string | Yes | `"theological-work"`, `"catechism-prose"`, `"treatise"`, `"devotional-classic"`, `"systematic-theology"`, `"church-history"`, `"martyrology"` |
| `sections` | array | Yes | Recursive tree |
| `sections[].section_type` | string | Yes | `"book"`, `"part"`, `"chapter"`, `"section"`, `"subsection"`, `"letter"`, `"preface"`, `"introduction"`, `"conclusion"`, `"appendix"` |
