...
def main() -> None:
    ...
    # Commentary OSIS failures are hard errors (not warnings) -- see validate.py line ~369.
    osis_errors = [e for e in errors if "OSIS existence" in e]
    check(
        "Bad cross_reference OSIS triggers existence error",
        len(osis_errors) == 1,
        f"errors={errors}",
    )
    ...
    osis_errors = [e for e in errors if "OSIS existence" in e]
    check(
        "Valid cross_reference OSIS produces no existence error",
        len(osis_errors) == 0,
        f"unexpected errors: {osis_errors}",
    )
    ...
    # When the apocrypha index is available, refs validate normally and return (True, "").
    # When it is unavailable, the code returns (True, "deuterocanonical ...").
    # Either way, the ref must be valid and must not be rejected as "unknown book code".
    for ref in ["Tob.1.1", "Sir.24.1", "1Macc.2.1", "Wis.7.1", "Bar.3.1", "PrAzar.1.1"]:
        ...
        check(
            ...
            valid and "unknown book code" not in reason,
