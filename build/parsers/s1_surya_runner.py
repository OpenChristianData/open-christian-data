"""Run Surya S1 page OCR sidecars through the engine venv subprocess."""
...
from build.lib.ocr_throttle import (  # noqa: E402
    THROTTLE_CHOICES,
    subprocess_kwargs_for_throttle as _subprocess_kwargs_for_throttle,
)
...
RUNNER_SCRIPT = Path("build") / "tools" / "ocr_runners" / "surya_page.py"
...
def _build_manifest_snapshot(
    ...
    failed_pages: int,
    created_at: str,
    bundle_extras_carried: dict[str, Any] | None = None,
) -> dict[str, Any]:
    extras = bundle_extras_carried if bundle_extras_carried is not None else {}
    return {
        ...
        "bundle_extras_carried": extras,
        "bundle_extras_carried_keys": sorted(extras),
        "bundle_extras_jcs_sha256": _extras_hash(extras) if extras else EMPTY_EXTRAS_SHA256,
...
# Inline Python that exercises the same initialization path surya_page.py uses.
_SURYA_PREFLIGHT_CMD = (
...
def _preflight_surya() -> None:
    """Initialize the Surya predictor stack once before the page loop.

    Any failure here — missing model weights, download timeout, import
    error — raises RuntimeError so no pages are baked into the run state.
    Mirrors the Calamari/Kraken preflight pattern.
    """
    engine_spec = next(spec for spec in ENGINE_SPECS if spec.name == ENGINE_FAMILY)
    cmd = [str(venv_python(engine_spec)), "-c", _SURYA_PREFLIGHT_CMD]
    try:
        subprocess.run(
            cmd,
            capture_output=True,
        ...
        raise RuntimeError(
            "Surya preflight failed: predictor initialization did not complete.\n"
            "Check network access and model cache, then retry.\n"
...
def _build_throttle_kwargs(throttle_mode: str, extra_env: dict[str, str] | None) -> dict:
    """Merge throttle settings and optional extra env vars into subprocess kwargs."""
    kwargs = _subprocess_kwargs_for_throttle(throttle_mode)
    if extra_env:
        base = kwargs.get("env") or {**os.environ}
        kwargs["env"] = {**base, **extra_env}
    return kwargs
...
def _run_page(
    ...
    throttle_kwargs = _build_throttle_kwargs(throttle_mode, extra_env)
    try:
...
def _run_page_batch(
    ...
    extra_env: dict[str, str] | None = None,
    max_width: int | None = None,
) -> list[tuple[dict[str, Any] | None, str | None, str | None]]:
    """Call surya_page.py --images with N paths; return one (payload, fc, err) per input.

    Passes all images to a single predictor call so model-load cost is paid
    once per batch.
    """
    engine_spec = next(spec for spec in ENGINE_SPECS if spec.name == ENGINE_FAMILY)
    max_width_args = ["--max-width", str(max_width)] if max_width is not None else []
    cmd = (
        [str(venv_python(engine_spec)), str(RUNNER_SCRIPT), "--images"]
        + [str(p) for p in image_paths]
        + max_width_args
        + ["--raw-outputs"]
        + [str(p) for p in raw_output_paths]
    )
    throttle_kwargs = _build_throttle_kwargs(throttle_mode, extra_env)
    try:
        result = subprocess.run(
        ...
        return [(None, "subprocess_timeout", err)] * len(image_paths)
    except subprocess.CalledProcessError as exc:
        stdout_text = (exc.stdout or b"").decode("utf-8", errors="replace")
        stderr_text = (exc.stderr or b"").decode("utf-8", errors="replace")
        lines = [ln.strip() for ln in stdout_text.splitlines() if ln.strip()]
        results: list[tuple[dict[str, Any] | None, str | None, str | None]] = []
        for ln in lines:
            try:
                p = json.loads(ln)
                results.append((None, str(p.get("failure_class") or "subprocess_error"), str(p.get("error") or "")))
            except json.JSONDecodeError:
                results.append((None, "subprocess_error", ln[:200]))
        while len(results) < len(image_paths):
            results.append((None, "subprocess_error", stderr_text.strip()[:200]))
    ...
    for ln in lines:
        try:
            ...
            results.append((None, "json_parse_error", f"{type(exc).__name__}: {str(exc)[:200]}"))
            continue
        if not payload.get("ok"):
            results.append((None, str(payload.get("failure_class") or "subprocess_error"), str(payload.get("error") or "")))
        else:
            results.append((payload, None, None))
    while len(results) < len(image_paths):
        results.append((None, "batch_output_truncated", f"expected {len(image_paths)} lines, got {len(lines)}"))
    return results[: len(image_paths)]
...
def normalize_volume(
    *,
    ...
    """Normalize one volume of Surya S1 page sidecars.

    batch_size: images passed to surya_page.py per subprocess call (default 1
    for backward-compatible single-image mode; >1 amortises model-load cost).
    recognition_batch_size: injects RECOGNITION_BATCH_SIZE env var (surya-ocr
    0.17.1 dynamic default: 32 on CPU).
    detector_batch_size: injects DETECTOR_BATCH_SIZE env var (dynamic default: 8 on CPU).
    max_width: downscale images wider than this (px) before inference; bboxes
    are scaled back to native coordinates by surya_page.py. 2500 gives a ~2x
    speedup on 5034px-wide NSH scans with no measurable quality loss.
    Recorded in the manifest bundle_extras_carried so the run is auditable.
    """
    ...
    _preflight_surya()
    if max_width is not None:
        print(f"  surya: max_width={max_width}px (downscaling active)", flush=True)
    ...
    # Build extra env vars for RECOGNITION/DETECTOR batch sizes.
    # Verified names against surya-ocr 0.17.1 (surya/settings.py).
    _extra_env: dict[str, str] = {}
    if recognition_batch_size is not None:
        _extra_env["RECOGNITION_BATCH_SIZE"] = str(recognition_batch_size)
    if detector_batch_size is not None:
        _extra_env["DETECTOR_BATCH_SIZE"] = str(detector_batch_size)
    _extra_env_arg: dict[str, str] | None = _extra_env if _extra_env else None

    if batch_size > 1:
        # Batch path: pre-compute per-image metadata, group non-skipped images
        # into chunks, call _run_page_batch once per chunk.
        all_meta = []
        for index, image_path in enumerate(images, start=1):
            ps = _page_sequence(index, image_path)
            pnid = image_path.stem
            pp = pages_dir / f"{pnid}.json"
            rp = _raw_artifact_path(pages_dir, pnid, ".surya.raw.json")
            sha = _prefixed_sha256_bytes(image_path.read_bytes())
            all_meta.append({
                ...
                "canonical_leaf_id": _leaf_id_for(sha),
                "edition_page_key": _edition_key_for(sha),
            })

        # R6a pre-run reuse summary -- before any engine call. Reuses the shas
        # already computed into all_meta (no extra image I/O).
        _reused = sum(
            1
            for m in all_meta
            if not force
            and _sidecar_is_done(
                m["page_path"],
                canonical_leaf_id=m["canonical_leaf_id"],
                source_payload_sha256=m["source_payload_sha256"],
            )
        )
        print(format_prerun_summary(SOURCE_LINEAGE_ID, volume, len(all_meta), _reused), flush=True)

        def _append_page_ref(m: dict[str, Any], failure_class: str | None) -> None:
            page_ref = {
                "page_native_id": m["page_native_id"],
                "page_sequence": m["page_sequence"],
                "status": "corrupt" if failure_class else "eligible",
                "sidecar_page_path": _relative_path(m["page_path"], repo_root),
                "source_payload_sha256": m["source_payload_sha256"],
            }
            set_leaf_or_exempt(page_ref, m["canonical_leaf_id"])
            _stamp_edition_page_key(page_ref, m["edition_page_key"])
            if failure_class:
        ...
        def _shutdown_requested() -> bool:
            if shutdown_event is None or not shutdown_event.is_set():
                return False
            print(
                f"    {SOURCE_LINEAGE_ID} vol_{volume:02d}: {len(page_refs)}/{len(all_meta)}"
        ...
        def _record_skipped(m: dict[str, Any]) -> None:
            nonlocal skipped_pages, failed_pages
            skipped_pages += 1
            failure_class = None  # sidecar already confirmed clean by _sidecar_is_done
            if failure_class:
                failed_pages += 1
            _append_page_ref(m, failure_class)
            _write_json(manifest_path, _build_manifest_snapshot(
                ...
                bundle_extras_carried={"surya_max_width": max_width} if max_width is not None else None,
            ))
        ...
        def _record_processed(
            ...
            nonlocal engine_version, emitted_pages, failed_pages
            payload, failure_class, error = result
            if payload is None and not failure_class:
                failure_class = "subprocess_error"
                error = "batch subprocess returned no payload"
            if failure_class:
                ...
                page_record = _page_record(
                    ...
                    raw_artifact=_raw_artifact_ref(m["raw_path"], repo_root),
                )
            _stamp_edition_page_key(page_record, m["edition_page_key"])
            _validate("sidecar-page-v1", page_record)
            _write_json(m["page_path"], page_record)
            emitted_pages += 1
            if not failure_class:
                emitted_state.add(m["page_native_id"])
            _write_json(
                state_path,
                {
                    "manifest_id": manifest_id,
        ...
        def _flush_pending() -> bool:
            ...
            chunk = list(pending)
            pending.clear()
            results = _run_page_batch(
                [m["image_path"] for m in chunk],
                [m["raw_path"] for m in chunk],
                SUBPROCESS_TIMEOUT,
                throttle_mode=throttle_mode,
                extra_env=_extra_env_arg,
                max_width=max_width,
            )
            for m, r in zip(chunk, results):
                _record_processed(m, r)
        ...
        for m in all_meta:
            if _shutdown_requested():
                break
            already_emitted = not force and _sidecar_is_done(
                m["page_path"],
                canonical_leaf_id=m["canonical_leaf_id"],
                source_payload_sha256=m["source_payload_sha256"],
            )
            if already_emitted:
                ...
                pending.append(m)
                if len(pending) >= batch_size and _flush_pending():
                    break
        else:
            _flush_pending()

    else:
        # Default single-page path (backward-compatible).
        for index, image_path in enumerate(images, start=1):
            page_sequence = _page_sequence(index, image_path)
            page_native_id = image_path.stem
            page_path = pages_dir / f"{page_native_id}.json"
            raw_path = _raw_artifact_path(pages_dir, page_native_id, ".surya.raw.json")
            ...
            # Shutdown check -- state is persisted; fall through to manifest write.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Page subset: integers or inclusive ranges (e.g. --pages 10-20 or --pages 10 11 12).",
    )
    parser.add_argument(
        "--max-width", type=int, default=None, dest="max_width",
        metavar="PX",
        help="Downscale images wider than PX before inference; bboxes scaled back to native coords.",
    )
    parser.add_argument(
        "--throttle",
        choices=THROTTLE_CHOICES,
        default="full-speed",
        help="CPU throttle mode.",
    )
    parser.add_argument("--force", action="store_true", help="Re-process already-emitted pages.")
