...
# Capture SWORD ThML scripture reference tags before HTML is stripped.
_SCRIPREF_PATTERN = re.compile(
    r"<scripRef\b[^>]*\bpassage=[\"']([^\"']+)[\"'][^>]*>(.*?)</scripRef>",
    re.IGNORECASE | re.DOTALL,
)
_SWORD_PASSAGE_BOOK_BOUNDARY = re.compile(
    r"\s+(?=(?:\d\s*)?[A-Za-z]+\d?\s+\d+:\d+)"
)
_SWORD_PASSAGE_CHAPTER_BOUNDARY = re.compile(r"(?<=\d)\s+(?=\d+:\d+)")
_SWORD_PASSAGE_DOT_REF = re.compile(r"(?<=\s)(\d+)\.(\d+)")


def _clean_block(text: str) -> str:
    """Strip HTML tags from a text block and normalize whitespace.

    Preserves paragraph structure by converting <br /><br /> to \\n\\n
...
def extract_scripref_cross_references(html_content: str) -> list[dict]:
    """Return source-order scripture references from SWORD <scripRef> tags."""
    refs = []
    for match in _SCRIPREF_PATTERN.finditer(html_content):
        raw = match.group(1).strip()
        if not raw:
            raw = _clean_block(match.group(2))
        if not raw:
            continue
        normalized = _SWORD_PASSAGE_DOT_REF.sub(r"\1:\2", raw)
        normalized = _SWORD_PASSAGE_CHAPTER_BOUNDARY.sub(
            "; ", _SWORD_PASSAGE_BOOK_BOUNDARY.sub("; ", normalized)
        )
        refs.append({"raw": raw, "osis": parse_thml_refs(normalized)})
    return refs
...
def make_entry(
    ...
    cross_references = extract_scripref_cross_references(html_content)
...
def main() -> None:
    ...
    with open(CONFIG_PATH, encoding="utf-8") as f:
        config = json.load(f)
    validate_config_enums(config, "devotional")
