...
UNMAPPED_CREEDS_METADATA_FIELDS = (
    "OriginStory",
    "AlternativeTitles",
    "Location",
    "SourceAttribution",
...
# Valid tradition values — read directly from schemas/v1/catechism_qa.schema.json.
VALID_TRADITIONS = get_enum("catechism_qa", "meta", "tradition")


def _validate_configs() -> None:
    for slug, cfg in DOCUMENT_CONFIGS.items():
        for tradition in cfg.get("tradition", []):
            assert tradition in VALID_TRADITIONS, f"{slug}: invalid tradition value {tradition!r}"
        assert "full" in CATECHISM_QA__META__COMPLETENESS, f"{slug}: schema enum CATECHISM_QA__META__COMPLETENESS no longer contains 'full' -- check generated enums"
        assert (
            "automated" in CATECHISM_QA__META__PROVENANCE__PROCESSING_METHOD
        ), f"{slug}: invalid processing_method value 'automated'"
...
def map_original_language(metadata: dict) -> str | None:
    raw_language = (metadata.get("OriginalLanguage") or "").strip()
    if not raw_language:
        return None
    try:
        return CREEDS_LANGUAGE_CODES[raw_language]
    except KeyError as exc:
        raise ValueError(f"Unsupported Creeds.json OriginalLanguage {raw_language!r}") from exc


def build_provenance_notes(metadata: dict) -> str:
    notes = [
        "Proof texts from Creeds.json are already in OSIS format -- raw and osis fields are identical.",
    ]
    language_code = map_original_language(metadata)
    if language_code:
        notes.append(
            # Interim parser-level fix; the future works registry will own work-level language.
            f"OriginalLanguage mapped to meta.original_language as {language_code}."
        )
    dropped_fields = [
        field for field in UNMAPPED_CREEDS_METADATA_FIELDS
        if metadata.get(field) not in (None, "", [])
    ]
    if dropped_fields:
        notes.append(
            "Creeds.json metadata fields intentionally not carried because catechism_qa "
            f"has no schema home: {', '.join(dropped_fields)}."
        )
    return " ".join(notes)
...
def build_meta(
    ...
    original_language = map_original_language(metadata) or "en"
    ...
    return {
        ...
        "contributors": normalize_contributors(authors[1:]),
        "original_publication_year": pub_year,
        "language": "en",
        # Interim parser-level fix; the future works registry will own work-level original language.
        ...
        "provenance": {
            ...
            "notes": build_provenance_notes(metadata),
        },
    }
...
def process_catechism(
    ...
    if dry_run:
        log("DRY RUN: parsed and mapped source; output file not written")
        return errors == 0
...
def main() -> None:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Parse and map source data but do not write output files.",
    )
    ...
    if args.all:
        try:
            ...
            for idx, stem in enumerate(ALL_CATECHISMS, start=1):
                ...
                ok = process_catechism(source_file, output_file, doc_cfg, dry_run=args.dry_run)
                if ok:
                    total_ok += 1
                else:
                    total_fail += 1
        ...
        try:
            success = process_catechism(source_file, output_file, doc_cfg, dry_run=args.dry_run)
            elapsed = (datetime.now(timezone.utc) - start).total_seconds()
            log("")
            if success:
