...
def extract_collects_from_html(html_text: str, source_filename: str) -> list:
    ...
    for part in parts:
        ...
        # Log a warning so parse failures are visible rather than silently dropped.
        if not text or word_count(text) < 10:
            wc = word_count(text) if text else 0
            logging.warning(
                "Collect '%s' in %s has only %d words (threshold 10) -- skipping; "
                "check HTML structure if this is unexpected",
                title, source_filename, wc,
            )
            continue
...
def main() -> None:
    ...
    with open(CONFIG_PATH, encoding="utf-8") as fh:
        config = json.load(fh)
    validate_config_enums(config, "prayer")
