...
def _validate_configs() -> None:
    for slug, cfg in CATECHISM_CONFIGS.items():
        for tradition in cfg.get("tradition", []):
            assert tradition in CATECHISM_QA__META__TRADITION, f"{slug}: invalid tradition value {tradition!r}"
        if completeness := cfg.get("completeness"):
            assert completeness in CATECHISM_QA__META__COMPLETENESS, (
...
CATECHISM_CONFIGS = {
    "luther_small": {
        ...
        "meta": {
            ...
            "contributors": normalize_contributors(["Robert E. Smith (translator, 1994; revised 2002, 2004)"]),
    ...
    "baltimore_3": {
        ...
        "meta": {
            ...
            "author_birth_year": None,
            "author_death_year": None,
            "contributors": normalize_contributors(["Rev. Thomas L. Kinkead (supplementer)"]),
            "original_publication_year": 1885,
            "language": "en",
            "original_language": "en",
            "tradition": ["catholic"],
...
def parse_luther_small(body_lines: list, doc_id: str, log_lines: list) -> tuple:
    ...
    while idx < len(non_empty):
        ...
        if _LSC_QUESTION_RE.match(stripped) and stripped.endswith("?"):
            question = decode_pg_inline_markup(stripped)
            ...
            answer = decode_pg_inline_markup(normalize_whitespace(" ".join(answer_parts)))
            ...
            article_ctx = decode_pg_inline_markup(normalize_whitespace(" ".join(article_lines)))
...
def parse_baltimore(body_lines: list, catechism_num: str, doc_id: str, log_lines: list) -> tuple:
    ...
    while i < len(lines):
        ...
        if q_match:
            ...
            q_text = decode_pg_inline_markup(normalize_whitespace(q_text))
            ...
            if i < len(lines) and _BALT_A_RE.match(lines[i].strip()):
                ...
                while i < len(lines):
                    ...
                    a_text += " " + next_ls
                    i += 1
                a_text = decode_pg_inline_markup(normalize_whitespace(a_text))
            else:
...
def build_output(cfg: dict, entries: list, source_hash: str, has_gaps: bool = False) -> dict:
    ...
    meta["provenance"] = {
        ...
        "notes": append_pg_inline_markup_note(None),
    }
