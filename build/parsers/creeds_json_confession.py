...
ALL_CONFESSIONS = list(DOCUMENT_CONFIGS.keys())

# Valid tradition values — read directly from schemas/v1/doctrinal_document.schema.json.
VALID_TRADITIONS = get_enum("doctrinal_document", "meta", "tradition")

VALID_DOCUMENT_KINDS = DOCTRINAL_DOCUMENT__DATA__DOCUMENT_KIND
...
def build_provenance_notes(metadata: dict) -> str:
    ...
    if language_code:
        notes.append(
            "OriginalLanguage="
            f"{language_code} recorded from Creeds.json; doctrinal_document has no "
            "meta.original_language field, so this remains provenance until the works registry owns it."
        )
    dropped_fields = [
    ...
    if dropped_fields:
        notes.append(
            "Creeds.json metadata fields intentionally not carried because doctrinal_document "
...
def main() -> None:
    ...
    source_group.add_argument(
        "--all",
        action="store_true",
        help="Process all known confessions in DOCUMENT_CONFIGS",
    )
    parser.add_argument(
        "--document-id",
        metavar="ID",
    ...
    if args.all:
        try:
            total_ok = 0
            total_fail = 0
            total = len(ALL_CONFESSIONS)
            for idx, stem in enumerate(ALL_CONFESSIONS, start=1):
                log("")
                log(f"--- [{idx}/{total}] {stem} ---")
                source_file = REPO_ROOT / "raw" / "Creeds.json" / "creeds" / f"{stem}.json"
                doc_cfg = DOCUMENT_CONFIGS[stem]
                output_file = DATA_DIR / f"{doc_cfg['document_id']}.json"
                ok = process_confession(source_file, output_file, doc_cfg, dry_run=args.dry_run)
                if ok:
                    total_ok += 1
                else:
                    total_fail += 1
    ...
    # Run
    try:
        success = process_confession(source_file, output_file, doc_cfg, dry_run=args.dry_run)
        elapsed = (datetime.now(timezone.utc) - start).total_seconds()
        print("")
        if success:
