...
| build/parsers/standard_ebooks.py | 2026-03-31 | Opus review: No critical issues. Important fixes applied: dead in_poem parameter removed from _walk(); [1/N] progress counter added for --all mode. Deferred: expected_count not set in 8 of 9 SE configs (source files not on disk — populate by running --list-files when repos are re-cloned). |
...
| tests/test_naves_osis.py | 2026-03-31 | Opus review: 3 new tests added — TestMakeUniqueId (no collision, first collision, second collision). Cross-ref-only assertion clarified: asserts all subtopics have empty references (implementation returns {'label':'See', 'references':[]} not []). 23/23 pass. |
| build/parsers/naves_topical.py | 2026-03-31 | Opus review: No critical issues. Important: schema description updated to cover both Nave's and Torrey's. Deferred: import unicodedata/html inside functions (cosmetic); _get_download_date fallback silently returns today's date (add logging.warning deferred); data_entries alias unnecessary (cosmetic). |
