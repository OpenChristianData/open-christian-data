...
MODERNISED_SCHEMA = json.loads((REPO_ROOT / "schemas" / "v1" / "modernised_record.schema.json").read_text(encoding="utf-8"))
...
def main(argv: list[str]) -> int:
    ...
    if not Path("schemas/v1/modernised_record.schema.json").exists():
        os.chdir(REPO_ROOT)
    return 0
