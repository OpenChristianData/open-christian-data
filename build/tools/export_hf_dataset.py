...
from ocd_kernel.lib.atomic_io import AtomicWriteError, write_json_atomic
...
def _render_dataset_card(data_root: Path, records_by_config: Mapping[str, list[Path]]) -> str:
    ...
    replacement = "\n".join(rows)
    if R65_PLACEHOLDER in template:
        return template.replace(R65_PLACEHOLDER, replacement)
    coverage_section = "\n".join(
        [
            "",
            "## Separate original/modernised projection coverage",
            "",
            "This section is generated by `build/tools/export_hf_dataset.py`. It is not the "
            "authoritative whole-corpus HuggingFace export path.",
            "",
    ...
    return _original_modernised_frontmatter() + _strip_frontmatter(template).rstrip() + coverage_section
...
def _original_modernised_frontmatter() -> str:
    return "\n".join(
        [
            "---",
            "license: cc0-1.0",
            "language:",
            "- en",
            "pretty_name: Open Christian Data original/modernised projection",
            "configs:",
