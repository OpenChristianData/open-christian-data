"""Standard Protestant canon chapter and verse counts."""

from __future__ import annotations


_OSIS_BOOK_ORDER = ['Gen', 'Exod', 'Lev', 'Num', 'Deut', 'Josh', 'Judg', 'Ruth', '1Sam', '2Sam', '1Kgs', '2Kgs', '1Chr', '2Chr', 'Ezra', 'Neh', 'Esth', 'Job', 'Ps', 'Prov', 'Eccl', 'Song', 'Isa', 'Jer', 'Lam', 'Ezek', 'Dan', 'Hos', 'Joel', 'Amos', 'Obad', 'Jonah', 'Mic', 'Nah', 'Hab', 'Zeph', 'Hag', 'Zech', 'Mal', 'Matt', 'Mark', 'Luke', 'John', 'Acts', 'Rom', '1Cor', '2Cor', 'Gal', 'Eph', 'Phil', 'Col', '1Thess', '2Thess', '1Tim', '2Tim', 'Titus', 'Phlm', 'Heb', 'Jas', '1Pet', '2Pet', '1John', '2John', '3John', 'Jude', 'Rev']
...
_VERSE_COUNTS = {
    ...
    'Mal': [14, 17, 18, 6],
    'Matt': [25, 23, 17, 25, 48, 34, 29, 34, 38, 42, 30, 50, 58, 36, 39, 28, 26, 34, 30, 34, 46, 46, 38, 51, 46, 75, 66, 20],
    'Mark': [45, 28, 35, 41, 43, 56, 36, 38, 48, 52, 32, 44, 37, 72, 46, 20],
    'Luke': [80, 52, 38, 44, 39, 49, 50, 56, 62, 42, 54, 59, 35, 35, 32, 31, 36, 43, 48, 47, 38, 71, 55, 53],
    'John': [51, 25, 36, 54, 46, 71, 53, 59, 41, 42, 57, 50, 38, 31, 27, 33, 26, 40, 42, 31, 25],
    'Acts': [26, 47, 26, 37, 42, 15, 60, 39, 43, 48, 30, 25, 52, 28, 40, 40, 34, 28, 41, 38, 40, 30, 35, 26, 27, 32, 44, 30],
...
def book_chapter_count(osis_id: str) -> int:
    """Return the number of chapters in a canonical Protestant book."""
    try:
        return len(_VERSE_COUNTS[osis_id])
    except KeyError as exc:
        raise ValueError(f"Unknown OSIS book id: {osis_id}") from exc


def book_verse_count(osis_id: str, chapter: int) -> int:
    """Return the number of verses in a canonical Protestant chapter."""
    if chapter < 1:
        raise ValueError("chapter must be >= 1")
    try:
        return _VERSE_COUNTS[osis_id][chapter - 1]
    except KeyError as exc:
        raise ValueError(f"Unknown OSIS book id: {osis_id}") from exc
    except IndexError as exc:
        raise ValueError(f"Unknown chapter for {osis_id}: {chapter}") from exc


def all_osis_books() -> list[str]:
    """Return canonical OSIS book ids in Protestant OT + NT order."""
    return list(_OSIS_BOOK_ORDER)
